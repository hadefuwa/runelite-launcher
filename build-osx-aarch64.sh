...
APPBASE="build/macos-aarch64/Alter.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/Alter.jar
    ...
    cp native/build-aarch64/src/Alter $APPBASE/Contents/MacOS/
    cp build/libs/Alter.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Alter
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Alter
    popd

    otool -l $APPBASE/Contents/MacOS/Alter
}
...
dmg() {
    ...
    mv Alter\ *.dmg Alter-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Alter-aarch64.dmg

    if ! hdiutil imageinfo Alter-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Alter-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Alter-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Alter-aarch64.dmg
    fi
}
